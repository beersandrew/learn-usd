...
if __name__ == "__main__":    
    ...
    origin_marker = build_item("blueSphere", "base_sphere", "sphere", 0, 0, 0, 1, 0, 0, 1)

    file_contents = file_header() + origin_marker

    for x in range(count):
        file_contents = file_contents + build_item("sphere_" + "{id}".format(id=x), "base_sphere", "sphere", random.uniform(minX, maxX), random.uniform(minY, maxY), random.uniform(minZ, maxZ), random.uniform(minRadius, maxRadius), random.random(), random.random(), random.random())
    print(file_contents)
